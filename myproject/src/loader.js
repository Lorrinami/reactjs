...
export const getFriendIdsForUser = userSouce => {
  const table = tables.usersFriends;
  const query = table
    .select(table.user_id_b)
    .where(table.user_id_a.equals(userSouce.id))
    .toQuery();
...
export const getUserNodeWithFriends = nodeId => {//性能优化的代码
  const { tableName, dbId } = tables.splitNodeId(nodeId);
...
// export const getPostIdsForUser = (userSource, args) => {//未添加验证
...
export const getPostIdsForUser = (userSource, args, context) => {
    ...
    return Promise.all([database.getSql(query.toQuery()),getFriendshipLevels(context)]).then(([allRows, friendshipLevels]) => {
